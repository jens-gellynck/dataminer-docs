...
The *MustSquashAlarms* tag is available from DataMiner 10.0.12 onwards. Set the value to *true* to enable [alarm consolidation](## "Also known as Alarm Squashing") by default. In that case, consecutive alarm events without a severity change will be combined into a consolidated event. This may be useful to reduce the load on DataMiner Cube and on the SLNet process.
